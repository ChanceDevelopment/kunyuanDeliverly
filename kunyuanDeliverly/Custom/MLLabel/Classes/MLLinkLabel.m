    ...
    for (NSRegularExpression *regexp in regexps) {
        [regexp enumerateMatchesInString:plainText options:0 range:textRange usingBlock:^(NSTextCheckingResult *result, __unused NSMatchingFlags flags, __unused BOOL *stop) {
            ...
            if (linkType!=MLLinkTypeNone) {
                NSLog(@"%ld,%ld",result.range.length,result.range.location);
                NSRange range = result.range;
                if (self.dataDetectorTypes == MLDataDetectorTypePhoneNumber) {
                    @try {
                        range = NSMakeRange(result.range.location, [plainText length] - result.range.location);
                ...
                MLLink *link = [MLLink linkWithType:linkType value:[plainText substringWithRange:range] range:range];
                if (self.beforeAddLinkBlock) {
                    self.beforeAddLinkBlock(link);
                }
